...
boost::shared_ptr<EncryptionContext> try_decrypt_header(const Poseidon::StreamBuffer &encrypted, const std::string &key){
	...
	if(expected_md5 != header.auth_md5){
		LOG_MEDUSA_DEBUG("Unexpected MD5: expecting ", Poseidon::hex_encode(expected_md5.data(), expected_md5.size()),
			", got ", Poseidon::hex_encode(header.auth_md5.data(), header.auth_md5.size()));
		return VAL_INIT;
	}
